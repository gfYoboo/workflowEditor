...
function GetConListFromNodeAtoNodeB(nodeA, nodeB) {
    let conList = [];
    store.state.ConditionList.forEach(con => {
        let nodeFrom = store.state.NodeList.find(node => node.DBID == con.DBID);

        if (nodeFrom != null && nodeFrom.DBID == nodeA.DBID) {
            let nodeTo = store.state.NodeList.find(node => node.DBID == con.DBID)
...
function ExistloopOfWorkFlow() {
    ...
    let workFlowNodeList = JSON.parse(JSON.stringify(store.state.NodeList));
    let workFlowConditionList = JSON.parse(JSON.stringify(store.state.ConditionList));


    let topologyNodeList = [];
    let NeedDeletedConTemp = [];

    let NeedContinue = true;//判断是否继续循环检测--如果检测不出还有入度为0的节点，就停止循环检测
    while (NeedContinue) {
        let nodeHt = CalculateNodeInDegree(workFlowNodeList, workFlowConditionList);

        let IncludeNodeOfZeroInDegree = false;//是否包含入度为0的节点
        for (let de in nodeHt) {
            if (nodeHt[de] === 0) {
                IncludeNodeOfZeroInDegree = true;//标记

                topologyNodeList.push(de);
                let de_index = workFlowNodeList.findIndex(item => item.DBID === de);
                workFlowNodeList.splice(de_index, 1);
                ...
                NeedDeletedConTemp.forEach(con => {
                    let con_index = workFlowConditionList.findIndex(item => item.DBID === con.DBID);
                    workFlowConditionList.splice(con_index, 1);//
                })
