<template>
  <el-form label-width="100px">
    <el-form-item label="审批用户描述">
      <el-select v-model="node.TitleDescription"
                 clearable
                 placeholder="">
        <template v-for="(item,index) in NodeUserDesList"
                  :key="index">
    ...
    <el-form-item label="选择审批用户">
      <el-select v-model="node.AppointUser"
                 clearable
                 placeholder="">
    ...
    <el-form-item label="审批人表达式">
      <el-input v-model="node.AppointCheckerExpression"
                readonly>
        <template #append>
    ...
    <el-form-item label="审批人列表">
      <el-button type="success"
                 ...
                 plian
                 icon="el-icon-plus"
                 @click="showNodeUserDialog()">新增</el-button>
    </el-form-item>
  </el-form>
  <el-table :data="node.NodeUserList"
            height="200"
            border
            style="width: 100%">
    ...
    <el-table-column prop="ComName"
                     label="单位名称"
                     width="90"
                     :show-overflow-tooltip="true">
    </el-table-column>
    <el-table-column label="是否本单位审批"
                     width="120">
    ...
    <el-table-column label="操作">
      <template #default="scope">
        <el-button type="primary"
                   ...
                   circle
                   size="mini"
                   @click="showNodeUserDialog(scope.row)"></el-button>
        <el-button type="danger"
                   icon="el-icon-delete"
                   circle
                   size="mini"
                   @click="delNodeUser(scope.row)"></el-button>
      </template>
    </el-table-column>
  </el-table>

  <nodeUserEdit ref="nodeUserEdit"
                @save="saveNodeUser"></nodeUserEdit>
</template>
  ...
  methods: {
    ...
    saveNodeUser(data) {
      ...
      if (data["ID"] < 0) {
        ...
        let index = this.node.NodeUserList.findIndex(d => d.UserCode === data['UserCode']);
